...
export function Tests() {
  ...
  if (taking) return <TakeTest test={taking} onDone={(score) => { actions.update('test_attempts', (rows) => [...rows, { id: `at-${Date.now()}`, test_id: taking.id, student_id: student.id, score, total: taking.question_ids.length, submitted_at: new Date().toISOString() }]); notify(`Submitted — you scored ${score}/${taking.question_ids.length}`); actions.push(['parent', 'teacher'], 'Online test submitted', `${taking.title}: ${score}/${taking.question_ids.length}`, 'homework', ['push']); setTaking(null); }} onCancel={() => setTaking(null)} />;
  return (
    <div>
